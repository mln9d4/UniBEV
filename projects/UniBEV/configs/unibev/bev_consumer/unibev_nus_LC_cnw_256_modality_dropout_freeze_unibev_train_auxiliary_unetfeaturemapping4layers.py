...
eval_interval = 2  # Reduced to every 2 epochs to save memory
samples_per_gpu = 1
workers_per_gpu = 1  # Reduced to free memory
max_epochs = 22
save_interval = 1
log_interval = 1
...
work_dir = './outputs/train/unibev_nus_LC_cnw_256_modality_dropout_freeze_unibev_train_auxiliary_unetfeaturemapping4layers'
...
model = dict(
    ...
    pts_bbox_head=dict(
        type='UniBEV_Head',
        freeze_unibev=True, # freeze the UniBEV model Mings code
        bev_h=bev_h_,
        bev_w=bev_w_,
        num_query=900,
        num_classes=10,
            ...
            decoder=dict(
                ...
                transformerlayers=dict(
                    ...
                    operation_order=('self_attn', 'norm', 'cross_attn', 'norm',
                                     'ffn', 'norm')))),
        bev_consumer=dict(
            type='UnetFeatureMapping4layers',
    ...
    hooks=[
        ...
        dict(type='WandbLoggerHook',
             init_kwargs=dict(
                 ...
                 name='unet_feature_mapping_4layers',
                 config=dict(
