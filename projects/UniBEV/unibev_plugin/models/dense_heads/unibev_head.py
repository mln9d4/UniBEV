...
class UniBEV_Head(DETRHead):
    """Head of UniBEV, based on Detr3D.
    Args:
    ...
    def __init__(self,
        ...
        if transformer['fusion_method'] == 'cat':
            self.scale_factor = 2 # used to scale up the dimension when concatenate
        else:
            self.scale_factor = 1

        self.dual_queries = transformer.get('dual_queries', False)
        ...
        super(UniBEV_Head, self).__init__(
    ...
    def _init_layers(self):
        ...
        for _ in range(self.num_reg_fcs):
            cls_branch.append(Linear(self.embed_dims * self.scale_factor, self.embed_dims * self.scale_factor))
            cls_branch.append(nn.LayerNorm(self.embed_dims * self.scale_factor))
            cls_branch.append(nn.ReLU(inplace=True))
        cls_branch.append(Linear(self.embed_dims * self.scale_factor, self.cls_out_channels))
        fc_cls = nn.Sequential(*cls_branch)

        reg_branch = []
        for _ in range(self.num_reg_fcs):
            reg_branch.append(Linear(self.embed_dims * self.scale_factor, self.embed_dims * self.scale_factor))
            reg_branch.append(nn.ReLU())
        reg_branch.append(Linear(self.embed_dims * self.scale_factor, self.code_size))
        ...
        if not self.as_two_stage:
            if self.dual_queries:
                self.bev_embedding_img = nn.Embedding(
                    self.bev_h * self.bev_w, self.embed_dims)
                self.bev_embedding_pts = nn.Embedding(
                    self.bev_h * self.bev_w, self.embed_dims)
            else:
    ...
    def forward(self, mlvl_img_feats, pts_feats, img_metas):
        """Forward function.
        Args:
            mlvl_feats (tuple[Tensor]): Image features extracted from the upstream
                camera branch network, each is a 5D-tensor with shape
                (B, N, C, H, W).

            pts_feats (tuple[Tensor]): BEV point cloud features extracted from the upstream
                lidar branch network, each is a 4D-tensor with shape
                (B, C, H, W)
        Returns:
        ...
        """
        if mlvl_img_feats is not None:
            bs, num_cam, _, _, _ = mlvl_img_feats[0].shape
            dtype = mlvl_img_feats[0].dtype
        elif pts_feats is not None:
            bs, _, _, _ = pts_feats[0].shape
            dtype = pts_feats[0].dtype

        object_query_embeds = self.query_embedding.weight.to(dtype)
        if self.dual_queries:
            bev_queries = [self.bev_embedding_img.weight.to(dtype), self.bev_embedding_pts.weight.to(dtype)]
            device = bev_queries[0].device
        else:
...
@HEADS.register_module()
class UniBEV_Head_bevconsumer(DETRHead):
    """Head of UniBEV, based on Detr3D.
    Args:
        with_box_refine (bool): Whether to refine the reference points
            in the decoder. Defaults to False.
    ...
    def __init__(self,
        ...
        super(UniBEV_Head_bevconsumer, self).__init__(
            *args, transformer=transformer, **kwargs)
